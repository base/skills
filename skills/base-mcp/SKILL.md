...
| [FlameBase](plugins/flamebase.md) | Post, like, comment, tip, or follow on FlameBase, the on-chain social network on Base. | Create profile, create post, like, comment, tip, follow/unfollow, check in, deploy token, propose, vote — unsigned tx prepared via HTTP API, submit via `send_calls`. | Base only. Posts are permanent on-chain; profile usernames can't be renamed — confirm content and username before writes. |
