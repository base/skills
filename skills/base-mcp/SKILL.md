...
| [Extra Finance xLend](plugins/extrafi-xlend.md) | Lend, borrow, repay, or withdraw on Extra Finance xLend. | Market/rate reads, positions and health factor, supply, withdraw, borrow, repay via `send_calls`. | Base only. CLI-only — requires shell access; stop on chat-only surfaces. Borrowing can be liquidated; surface health factor before risky actions. |
