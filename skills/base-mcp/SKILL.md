...
| [GBLIN](plugins/gblin.md) | Move surplus USDC into the GBLIN treasury index, redeem it back to USDC, or check the market risk regime before deploying capital. | Treasury/NAV reads, quotes, invest calldata, JIT redeem to USDC, risk attestations. | Base only. Endpoints are x402-paid (USDC per call) and return unsigned calldata only; NAV moves with WETH/cbBTC  it is not a stablecoin. |
