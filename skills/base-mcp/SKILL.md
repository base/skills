...
| [x402 Bazaar](plugins/x402-bazaar.md) | Get onchain data or AI reports on Base — token risk/safety, wallet net worth & activity, OFAC sanctions, prices, NFTs. | Token risk & rug score, AI token/wallet reports, OFAC screening, net worth, approvals, transfers, prices, NFT floor. | Base only. Read-only (no transactions); paid per call in USDC over x402 via the x402-bazaar-mcp server. |
