...
| [x402 Bazaar](plugins/x402-bazaar.md) | Get onchain data or AI reports on Base — token risk/safety, the B20 (Base-native token) safety suite, wallet drain-surface audits, OFAC sanctions, prices, NFTs. | Token risk & rug score, B20 freeze/seize suite, AI token/wallet reports, OFAC screening, approvals + spend permissions + 7702 delegation audits, prices, NFT floor. | Base only. Read-only (no transactions); paid per call in USDC over x402 via the x402-bazaar-mcp server. |
